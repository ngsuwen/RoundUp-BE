...
router.get("/expense/user/:usernameid/:monthOfExpense", async (req,res)=>{
    ...
    const expense = await DataExpense.find({username:usernameid, 'expensesentry.date':{'$gte': new Date(`${monthOfExpense}-01`), '$lt': new Date(`${monthOfExpense}-31`).setHours(23,59,59,999)}})
    res.send(expense)
})
